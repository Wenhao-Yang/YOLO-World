...
@DATASETS.register_module()
class YOLOv5MixedGroundingAudioDataset(BatchShapePolicyDataset, CocoDataset):
    """Mixed grounding dataset."""
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        audios = []
        cat2id = {}
        audio_prefix = img_prefix.replace('images', 'audios')
        for ann in ann_info:
            cat_name = ' '.join([img_info['caption'][t[0]:t[1]]
                                 for t in ann['tokens_positive']])
            if cat_name not in cat2id:
                cat2id[cat_name] = len(cat2id)
                try:
                    for an in ann['audio_tokens']:
                        an[0] = osp.join(audio_prefix, an[0])
                except Exception as e:
                    print(ann)
                    raise e
                # print(ann)
                audios.append(ann['audio_tokens'])
                
#                 for i, text_token in enumerate(img_info['tokens_positive_eval']):
#                     if ann['tokens_positive'] == text_token:
#                         if 'audio_tokens' in img_info:
#                             audio_token = img_info['audio_tokens'][i].copy()
#                             if isinstance(audio_token[0], list):
#                                 for a in audio_token:
#                                     a[0] = osp.join(audio_prefix, a[0])
#                                 audio_token = audio_token[0]
#                             else:
#                                 audio_token[0] = osp.join(audio_prefix, audio_token[0])

#                             audios.append(audio_token)
        
        if len(audios) > 0:
            data_info['audio'] = audios
